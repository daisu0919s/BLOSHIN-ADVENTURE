...
class App:
    def __init__(self):
        pyxel.init(128, 128, title="BLO-SHIN ADVENTURE")
...
def game_over():
    ...
    def draw_game_over_screen():
        ...
        # 中央に配置するための座標を計算
        text1 = "BLO-SHIN IS EATEN!"
        text2 = f"Score: {score}"
        text3 = "Press R or B to restart"
        text1_x = (pyxel.width - len(text1) * pyxel.FONT_WIDTH) // 2
        text2_x = (pyxel.width - len(text2) * pyxel.FONT_WIDTH) // 2
        text3_x = (pyxel.width - len(text3) * pyxel.FONT_WIDTH) // 2
        text_y = pyxel.height // 2

        # 点滅テキスト: フレーム数で色を切り替え
        color = (pyxel.frame_count // 4) % 16  # 点滅カラー
        pyxel.text(text1_x, text_y - 20, text1, color)  # ゲームオーバーメッセージ
...
def reset_game():
    """ゲームのリセット処理を分離"""
    global scroll_x, player, enemies
